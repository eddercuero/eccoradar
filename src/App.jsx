...
import { collection, addDoc, onSnapshot, query, orderBy, serverTimestamp, limit, doc, setDoc, getDoc } from "firebase/firestore";
...
export default function EcoRadar() {
  ...
  // la nube (porque antes solo vivían en este navegador), los recuperamos de
  // aquí y los subimos, en vez de asumir que están vacíos. Se marca con
  // "migracionCompleta" para que esto pase UNA sola vez por institución y
  // nunca vuelva a interferir con un vaciado real que hagas después.
  useEffect(() => {
    if (!sesion?.empresaId) return;
    let cancelado = false;
    (async () => {
      try {
        const ref = doc(db, "app_data", sesion.empresaId);
        const snap = await getDoc(ref);
        const datos = snap.exists() ? snap.data() : {};
        if (datos.migracionCompleta) return;
        const noVacio = (v) => Array.isArray(v) ? v.length > 0 : !!(v && Object.keys(v).length > 0);
        const personasRescatadas = (cargar("eco_gad_personas", []) || []).map(p => (p.rol === "Directora" ? { ...p, rol: ROL_DIRECTORA } : p));
        const campos = {
          personas: noVacio(datos.personas) ? datos.personas : personasRescatadas,
          tareas: noVacio(datos.tareas) ? datos.tareas : cargar("eco_gad_tareas", []),
          metas: noVacio(datos.metas) ? datos.metas : cargar("eco_gad_metas", []),
          turnos: noVacio(datos.turnos) ? datos.turnos : cargar("eco_gad_turnos", []),
          proyectos: noVacio(datos.proyectos) ? datos.proyectos : cargar("eco_gad_proyectos", []),
          contenidoPlan: noVacio(datos.contenidoPlan) ? datos.contenidoPlan : cargar("eco_gad_contenido_plan", []),
